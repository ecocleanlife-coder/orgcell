...
export default function DropboxCallback() {
    ...
    const [status, setStatus] = useState('Dropbox 연결 중...');
    ...
    useEffect(() => {
        ...
        if (error) {
            toast.error('Dropbox 연결이 거부되었습니다.');
            navigate('/family-setup', { replace: true });
            return;
        }

        if (code) {
            axios.post('/api/dropbox/callback', { code })
                .then(res => {
                    if (res.data.success) {
                        toast.success('Dropbox가 성공적으로 연결되었습니다!');
                        navigate('/family-setup', { replace: true });
                    } else {
                ...
                .catch(err => {
                    ...
                    toast.error('Dropbox 연결 중 오류가 발생했습니다.');
