...
### 24-6. 자녀 간격 (§24-6)
- 자녀 배치 시 블록 너비 기준으로 위치 계산 (고정 step 방식 폐기)
- 단독 자녀 블록 너비 (SINGLE_BLOCK_W): CARD_WIDTH(220) + BLOCK_PADDING×2(16) = 236px
- 부부 자녀 블록 너비 (COUPLE_WIDTH): 440px
- 블록 간 최소 간격 (CHILD_GAP): **40px** (카드 끝-끝 기준)
- 배치 방식: 전체 자녀 블록 합계 너비 + 간격 합 = 총 너비 → 부모 중심 기준 좌우 대칭 배치
